package com.disanyuzhou.htmlparse.conf
    ...
    for (url <- urls) {
      ...
      for (q <- queries) {
        ...
        for (extract <- extracts) {
          ...
          val format = (extract \ "@format").text
          query.addItem(new Command(extractType, extractField, format, exist, extractParams))
        }
        queryArray(i) = query
        i += 1
      }
...
case class Command(val name: String, val filed: String, format: String = "", val mustHave: Boolean = true, val params: String = "")
